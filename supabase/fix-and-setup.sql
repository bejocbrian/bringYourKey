-- ==========================================
-- Fix & Setup Admin Schema
-- Run this in Supabase SQL Editor to cleanly set up the admin panel
-- ==========================================

-- 1. Helper Functions (Fixes RLS Recursion)
-- Defined first so policies can use them
...
-- ==========================================
-- 2. Clear Existing Policies on Profiles (To prevent conflicts)
-- ==========================================
...
DROP POLICY IF EXISTS "Superadmins can update user roles" ON public.profiles;

-- ==========================================
-- 3. Re-apply Profile Policies (Base + Admin)
-- ==========================================
...
-- Admin Policies (Using non-recursive function)
...
-- ==========================================
-- 4. Admin Tables (Drop & Recreate to ensure clean state)
-- ==========================================
...
CREATE POLICY "Admins can modify feature flags" ON public.feature_flags FOR ALL USING (public.is_admin());
...
CREATE POLICY "Everyone can read provider configs" ON public.provider_configs FOR SELECT USING (true);
CREATE POLICY "Admins can modify provider configs" ON public.provider_configs FOR ALL USING (public.is_admin());
...
CREATE POLICY "Admins can modify app settings" ON public.app_settings FOR ALL USING (public.is_admin());
...
CREATE POLICY "Admins can read all generations" ON public.generations FOR SELECT USING (public.is_admin());
...
-- Provider Configs
INSERT INTO public.provider_configs (id, name, enabled, is_default, rate_limit_rpm, cost_per_generation, max_duration, supported_ratios) VALUES 
